...
class AlchemicalTransformation:
    ...
    def energy_derivative(self, context, energy_unit=unit.kilojoule_per_mole):
        ...
        The coupling is linear, so the partial derivative dE/dlambda = U(1) - U(0) exactly,
        which we can evaluate easily by taking the difference of the energy in the two states.
        
        Normally, we would also be able to use the openmm ``state.getEnergyParameterDerivatives()``,
        which rounds up the ``Force``s that have had derivatives added to them by ``addEnergyParameterDerivative``,
        but openmm doesn't support this method for ``CustomHbondForce``s (BasePair and CrossStacking),
        so actually we can't do this."""
...
class AlchemicalTransformation2:
    """A single-topology transformation of one or more DNA bases into new identities.
    ...
    dna : DNA
        The initial-state DNA (already built, with its topology computed).
    dna_target : DNA
        Can be created by ff3SPN2.DNA.create_mutant or any other method the user wishes to use
    lambda_name : str
        Base name for the coupling; the two state weights are ``f'{lambda_name}_initial'`` and
        ``f'{lambda_name}_target'``.
    """

    def __init__(self, dna_initial, dna_target, lambda_name='lambda'):
        self.initial = dna_initial
        self.target = dna_target
        self.lambda_name = lambda_name
    ...
    def add_forces(self, system, forces=None, verbose=False):
        """Build the hybrid on `system` (an ``openmm.System`` or ``open3SPN2.System``).
        ...
        forces: iterable
            List of Forces to add to the System. Defaults to open3SPN2.forces.
        """
        if forces is None:
            forces = open3SPN2.forces
        added = {}
        for force_name, ForceClass in forces.items():
            if verbose:
                print(force_name)

...
class BasePair(DNAForce, openmm.CustomHbondForce):
    ...
    def addForce(self, system, names=None):
        for counter, f in enumerate(self.forces):
            try:
                system.addForce(self.forces[f])
            except AttributeError:
                system.addCollectiveVariable(names[counter], self.forces[f]) # our "system" might actually be a CustomCVForce
...
class CrossStacking(DNAForce):
    ...
    def addForce(self, system, names=None):
        for counter, (c1, c2) in enumerate(self.crossStackingForces.values()):
            try:
                system.addForce(c1)
                system.addForce(c2)
            except AttributeError: # names should be a list of lists
                system.addCollectiveVariable(names[counter][0], c1) # our "system" might actually be a CustomCVForce
                system.addCollectiveVariable(names[counter][1], c2) # our "system" might actually be a CustomCVForce
